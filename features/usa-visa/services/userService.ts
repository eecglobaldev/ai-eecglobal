 ...
 * 2. Server creates Firebase Auth user, users_auth, and usa_users entry
 * 3. Update usa_users with additional fields (state name, branch name, etc.)
 * 
...
export const incrementPrepPlanCount = async (email: string) => {
  try {
    ...
    const userRef = doc(db, "usa_users", userDoc.id);
 ...
 * Passes 'usa_users' as the collection for migration.
 * 
...
export const signInWithHiddenAuth = async (
  ...
  return await globalSignInWithHiddenAuth(email, 'usa_users');
};
