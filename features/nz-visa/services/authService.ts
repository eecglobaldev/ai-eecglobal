 ...
 * - nz_users/{uid} (primary for this app)
 * - uk_users/{uid}
 * - legacy usa_users/{uid}
 */
 ...
 * 
 * @param email - User email
 * @param collection - Optional collection name for migration (default: nz_users)
 * @returns Firebase User or null
 */
...
export const ensureUserSignedIn = async (): Promise<User> => {
  try {
    ...
    const userEmail =
      localStorage.getItem("NZ_UserEmail") ||
      localStorage.getItem("NZUserEmail");
    if (!userEmail) {
