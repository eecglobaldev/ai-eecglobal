...
const NAV_LINKS = [
  { href: '/careercounselor/', text: 'Start' },
  { href: '/careercounselor/faq/', text: 'FAQ' },
  { href: '/careercounselor/glossary/', text: 'Glossary' },
  { href: '/careercounselor/preparation-guide/', text: 'Preparation Guide' },
  { href: '/careercounselor/resources/', text: 'Resources' },
  { href: '/careercounselor/about-eec/', text: 'About EEC' },
];
    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      >
        ...
        <div className="p-4 space-y-4">
          {/* Tool Pages */}
          <div>
            <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Pages</h3>
            <ul className="space-y-1">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="block px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300" onClick={onClose}>
                    {link.text}
                  </Link>
                </li>
              ))}
      ...
      >
        <nav className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8" aria-label="Main navigation">
          <div className="flex items-center justify-between flex-nowrap py-2 sm:py-0 sm:h-16">
            ...
            <div className="hidden lg:flex items-center gap-1 flex-1 justify-center mx-4 overflow-x-auto scrollbar-hide">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-3 py-1.5 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors whitespace-nowrap"
                >
                  {link.text}
                </Link>
              ))}
