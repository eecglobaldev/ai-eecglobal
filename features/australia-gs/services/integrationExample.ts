...
export const saveCompleteApplication = async (
  ...
  try {
    ...
    // 1. Ensure user is signed in with hidden Firebase Auth (passwordless)
    ...
    // 2. Get authenticated user email
    ...
    // 3. Get user ID (Firebase Auth UID)
    const userId = await getUserIdByEmail(userEmail);
    if (!userId) {
      showModal("User not found. Please log in again.");
      return false;
    ...
    // 6. Prepare optional transcript file
    ...
    // 7. Save everything to Firebase (Storage + Firestore)
    const result = await saveStudentApplication(
      userId,
      applicationData,
      sopFile,
...
/**
 * Save with transcript (for interview practice)
 */
export const saveWithInterviewData = async (
  ...
  return saveCompleteApplication(formData, showModal, setSaveProgress, transcriptText);
};
