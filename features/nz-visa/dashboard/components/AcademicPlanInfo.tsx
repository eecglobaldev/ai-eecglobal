...
import { AcademicPlanData, ApplicationData } from '../types';
...
const AcademicPlanInfo: React.FC<AcademicPlanInfoProps> = ({ academicPlanData, applicationData, theme = 'dark' }) => {
  ...
  if (!academicPlanData && !applicationData) {
    return (
  ...
  // Merge data from both sources (applicationData takes precedence for new fields)
  ...
  // Helper to get field value - check applicationData first, then academicPlanData, then careerGoals
  const getField = (fieldName: string): any => {
    // Check applicationData first
    if (applicationData && (applicationData as any)[fieldName] !== undefined && 
        (applicationData as any)[fieldName] !== null && (applicationData as any)[fieldName] !== '') {
      return (applicationData as any)[fieldName];
    }
    // Check academicPlanData top level
    if (academicPlanData) {
  ...
  const course = getField('course') || getField('courseName');
  const courseLevel = getField('courseLevel');
  const grade = getField('grade');
  const fundingSource = getField('fundingSource') || getField('primarySponsor');
  const hasPetition = getField('hasPetition');
  const hasRefusal = getField('hasRefusal') || getField('hasVisaRefusal');
  const hasTraveled = getField('hasTraveled');
  const institutionName = getField('institutionName');
  const institutionType = getField('institutionType');
  const maritalStatus = getField('maritalStatus');
  const previousEducation = getField('previousEducation');
  const refusalReason = getField('refusalReason');
  const testScoresFromApp = getField('testScores');
  ...
  return (
    <div className="bg-glass-bg p-4 sm:p-6 rounded-2xl sm:rounded-3xl backdrop-blur-lg border border-glass-border">
      ...
      <div className="space-y-6">
        {/* University */}
        {(academicPlanData?.university || institutionName) && (
          <div>
            <SectionHeader title="Target University/Institution" sectionKey="university" icon="🏛️" />
            {expandedSections.university && (
              <div className="pl-2">
                <div className="bg-gradient-to-r from-primary-purple/30 to-primary-purple/10 p-4 rounded-xl border-l-4 border-primary-purple">
                  <p className="text-lg font-bold text-white">{academicPlanData?.university || institutionName}</p>
                  {institutionType && (
                    <p className="text-sm text-gray-300 mt-2">Type: {institutionType}</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div>
          ...
          {expandedSections.basic && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-2">
              ...
              <InfoCard label="Previous Education" value={previousEducation} />
        ...
        {/* Test Scores */}
        {(testScores.ielts || testScores.toefl || testScores.pte || testScores.duolingo || testScores.gre || testScores.gmat || testScores.sat || testScoresFromApp) && (
          <div>
            ...
            {expandedSections.tests && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pl-2">
                ...
                {testScoresFromApp && !testScores.ielts && (
                  <InfoCard label="Test Score" value={testScoresFromApp} />
                )}
        ...
        {/* Career Goals */}
        {((academicPlanData?.careerGoals && (academicPlanData.careerGoals.goal || academicPlanData.careerGoals.details)) || getField('careerGoals')) && (
          <div>
            <SectionHeader title="Career Goals" sectionKey="career" icon="🎯" />
            {expandedSections.career && (
              <div className="pl-2 space-y-3">
                {(academicPlanData?.careerGoals?.goal || getField('careerGoals')) && (
                  <div className="bg-gradient-to-r from-green-500/20 to-transparent p-4 rounded-xl border-l-4 border-green-500">
                    ...
                    <p className="text-base font-medium leading-relaxed">{academicPlanData?.careerGoals?.goal || getField('careerGoals')}</p>
                  </div>
                )}
        ...
        {(maritalStatus || getField('childrenAccompanying') || getField('spouseAccompanying')) && (
          <div>
            <SectionHeader title="Personal & Family Information" sectionKey="personal" icon="👨‍👩‍👧‍👦" />
            {expandedSections.personal && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-2">
                <InfoCard label="Marital Status" value={maritalStatus} />
                <InfoCard label="Spouse Accompanying" value={getField('spouseAccompanying')} />
                <InfoCard label="Spouse Qualification" value={getField('spouseQualification')} />
                <InfoCard label="Spouse Experience" value={getField('spouseExperience')} />
                <InfoCard label="Marriage Duration (months)" value={getField('marriageDurationInMonths')} />
                <InfoCard label="Date of Marriage" value={getField('dateOfMarriage')} />
                <InfoCard label="Children Accompanying" value={getField('childrenAccompanying')} />
                <InfoCard label="Number of Children" value={getField('numberOfChildren')} />
                <InfoCard label="Children Ages" value={getField('childrenAges')} />
              </div>
            )}
          </div>
        )}
        ...
        <div>
          ...
          {expandedSections.background && (
            <div className="pl-2 space-y-3">
              ...
              {(academicPlanData?.refusalReason || refusalReason) && (
                <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-xl">
                  <p className="text-xs text-red-400 mb-2 font-semibold">Refusal Details</p>
                  {academicPlanData?.refusalType && (
                    <p className="text-sm mb-2"><strong>Type:</strong> {academicPlanData.refusalType}</p>
                  )}
                  <p className="text-sm leading-relaxed whitespace-pre-wrap">{academicPlanData?.refusalReason || refusalReason}</p>
                </div>
              )}
        ...
        {/* Financial Sponsors */}
        {(sponsors.length > 0 || getField('sponsor1Name') || getField('fundingSources') || fundingSource) && (
          <div>
              ...
              <div className="pl-2 space-y-4">
                ...
                {fundingSource && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                    <InfoCard label="Primary Sponsor" value={fundingSource} fullWidth />
                  </div>
                )}
                
                {/* Funding Sources from Application Data */}
                {getField('fundingSources') && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                    <p className="text-sm font-bold text-primary-white mb-3">Funding Sources</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {getField('fundingSources').familySavings && (
                        <InfoCard label="Family Savings" value={`${getField('fundingSources').familySavings} Lakhs`} />
                      )}
                      {getField('fundingSources').personalSavings && (
                        <InfoCard label="Personal Savings" value={`${getField('fundingSources').personalSavings} Lakhs`} />
                      )}
                      {getField('fundingSources').educationLoan && (
                        <InfoCard label="Education Loan" value={`${getField('fundingSources').educationLoan} Lakhs`} />
                      )}
                    </div>
                  </div>
                )}
                ...
                {/* Sponsor 1 */}
                {getField('sponsor1Name') && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/10 hover:border-primary-purple/50 transition-colors">
                    <p className="text-base font-bold text-primary-white mb-3">Sponsor 1</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <InfoCard label="Name" value={getField('sponsor1Name')} />
                      <InfoCard label="Profession" value={getField('sponsor1Profession')} />
                      <InfoCard label="Income" value={getField('sponsor1Income')} />
                    </div>
                  </div>
                )}

                {/* Sponsor 2 */}
                {getField('sponsor2Name') && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/10 hover:border-primary-purple/50 transition-colors">
                    <p className="text-base font-bold text-primary-white mb-3">Sponsor 2</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <InfoCard label="Name" value={getField('sponsor2Name')} />
                      <InfoCard label="Profession" value={getField('sponsor2Profession')} />
                      <InfoCard label="Income" value={getField('sponsor2Income')} />
                    </div>
                  </div>
                )}

                {/* Academic Plan Sponsors */}
        ...
        {/* Work Experience */}
        {(workExperience.length > 0 || getField('workExperience')) && (
          <div>
            <SectionHeader title="Work Experience" sectionKey="work" icon="💼" />
            {expandedSections.work && (
              <div className="pl-2 space-y-3">
                {/* Work experience from applicationData */}
                {getField('workExperience') && typeof getField('workExperience') === 'string' && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                    <p className="text-sm font-bold text-primary-white mb-2">Work Experience</p>
                    <p className="text-sm text-gray-300">{getField('workExperience')} years</p>
                  </div>
                )}
                
                {/* Work experience from academicPlanData */}
        ...
        {/* Timestamp */}
        {(academicPlanData?.timestamp || applicationData?.timestamp) && (
          <div className="pt-4 border-t border-glass-border">
            <p className="text-xs text-gray-400">
              Last Updated: {academicPlanData?.timestamp || applicationData?.timestamp ? new Date(academicPlanData?.timestamp || applicationData?.timestamp || 0).toLocaleString('en-US', { 
                dateStyle: 'medium', 
                timeStyle: 'short' 
              }) : 'N/A'}
            </p>
