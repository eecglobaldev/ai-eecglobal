        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const prepData: PrepData[] = prepDataSnapshot.docs.map(doc => {
                    const data = doc.data();
                    // Handle old format where type might be missing
                    const prepDataItem: PrepData = {
                        id: doc.id,
                        type: data.type || (data.keyTalkingPointsUrl ? 'kpoints' : 'sop'),
                        url: data.url || data.keyTalkingPointsUrl || '',
                        keyTalkingPointsUrl: data.keyTalkingPointsUrl,
                        createdAt: getSafeTimestampString(data.createdAt || data.timestamp) || new Date().toISOString(),
                    };
                    return prepDataItem;
