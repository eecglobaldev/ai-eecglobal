...
export const generatePrepPlan = async (profile: UserProfile): Promise<PrepContent | null> => {
    ...
    try {
        ...
        const prepContent = extractJson<PrepContent>(response.text);

        // Flatten sections into interview_questions for easy access
        if (prepContent && prepContent.sections) {
            prepContent.interview_questions = prepContent.sections.flatMap(section => section.questions || []);
        }
