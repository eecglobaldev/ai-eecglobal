...
const LoginSignupModal: React.FC<LoginSignupModalProps> = ({ onAuthSuccess, onSwitchToLogin, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    state: '',
    city: '',
    // targetCountry: 'United Kingdom', // Default to United Kingdom
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('AiCourseCounselor', formData.email);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-4 sm:p-5 relative max-h-[96vh] overflow-y-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        <div className="text-center mb-3">
          ...
          <p className="mt-1 text-lg text-slate-600 dark:text-slate-400">
            ...
            <button onClick={onSwitchToLogin} className="text-indigo-600 underline font-semibold hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors duration-300 cursor-pointer">
              Sign In
            </button>
          </p>
        </div>
