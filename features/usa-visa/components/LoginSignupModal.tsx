...
import { registerUser, checkEmailExists, checkUserExists } from "../services/userService";
...
const LoginSignupModal: React.FC<LoginSignupModalProps> = ({ onAuthSuccess, onSwitchToLogin, onClose }) => {
    ...
    const handleSendPhoneOtp = async () => {
        ...
        try {
            ...
            if (result && (result.success || result.type === 'success')) {
                setPhoneStep("pending");
                setCountdown(60);
            } else {
                const errorMsg = result?.message || result?.error || "Failed to send phone OTP. Try again.";
                console.error("❌ OTP Send Failed:", errorMsg);
                setPhoneError(errorMsg);
            }
        } catch (err: any) {
            console.error("🔥 Phone OTP Error:", err);
            const errorMsg = err?.response?.data?.error || err?.message || "Unable to send phone OTP. Try again.";
            setPhoneError(errorMsg);
        }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        // const checked = type === 'radio' ? (e.target as HTMLInputElement).checked : undefined;
    ...
    const handleSendEmailOtp = async () => {
        ...
        try {
            ...
            // If email doesn't exist, send OTP with target country
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const userDataToSave = {
                ...formData,
                targetCountry: formData.targetCountry || 'USA', // Default to USA if not set
            ...
            localStorage.setItem('USAUserEmail', formData.email);

            // 🔐 SIGN IN WITH HIDDEN AUTH (Passwordless Backend-Managed Auth)
            // After registration, automatically sign in the user with Firebase Auth
            ...
            try {
                ...
                if (firebaseUser) {
                    ...
                    console.warn("⚠️ Failed to sign in with hidden auth, but registration succeeded");
                }
            } catch (authError) {
                console.error("❌ Error signing in with hidden auth:", authError);
                // Don't block the flow if sign in fails - user is registered
            }
            ...
            // after filling the academic plan form
        ...
        `}</style>
                ...
                <div className="text-center mb-3">
                    ...
                    <p className="mt-1 text-md text-slate-600 dark:text-slate-400">
                        ...
                        <button onClick={onSwitchToLogin} className="text-indigo-600 underline">
                            Sign In
                        </button>
                    </p>
                </div>
              ...
              {emailVerificationStep === "verified" && (
                <span className="text-green-600 font-semibold">✔ Verified</span>
                    ...
                    <button
                        ...
                        className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg mt-4 transition-colors duration-200 disabled:bg-slate-400 disabled:cursor-not-allowed disabled:opacity-70 flex items-center justify-center min-h-[44px]"
                    >
