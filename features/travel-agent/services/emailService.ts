...
// Country-specific admin email mapping
// Add the actual email addresses for each country's admin
const COUNTRY_ADMIN_EMAILS: { [key: string]: string[] } = {
  'Germany': ['eeccouns58@gmail.com', 'eecparvatpatiya@gmail.com', 'eecvesu@gmail.com', 'eeccouns54@gmail.com'], // TODO: Add Germany country admin email(s)

  'Deutschland': ['eeccouns58@gmail.com', 'eecparvatpatiya@gmail.com', 'eecvesu@gmail.com', 'eeccouns54@gmail.com'], // Same as Germany (alternative name)

  'USA': ['eeccouns032@gmail.com',
      'eecnikolvisa@gmail.com',
      'eecvvnusa@gmail.com'], // TODO: Add USA country admin email(s)

  'UK': ['eeccouns017@gmail.com',
      'eeccouns026@gmail.com',
      'eecukahmvisa@gmail.com'], // TODO: Add UK country admin email(s)

  'Finland': ['eecghatlodiyavisa@gmail.com', 'eeccouns017@gmail.com'], // TODO: Add Finland country admin email(s)

  'New Zealand': ['eeccouns009@gmail.com',
      'eecnikolvisa@gmail.com',
      'eeccouns48@gmail.com',
      'eeccouns57@gmail.com'], // TODO: Add New Zealand country admin email(s)

  'Australia': ['eecaustralia01@gmail.com',
      'eeccouns55@gmail.com',
      'eeccouns35@gmail.com'], // TODO: Add Australia country admin email(s)

  'Canada': ['eeccouns005@gmail.com', 'eeccouns009@gmail.com', 'eecbharuchvisa@gmail.com'], // TODO: Add Canada country admin email(s)

  'UAE': ['eecvesu@gmail.com', 'eecvarachacanada@gmail.com', 'eecparvatpatiya@gmail.com', 'eeccouns57@gmail.com', 'eeccouns026@gmail.com'], // TODO: Add UAE/Dubai country admin email(s)
  
  'Dubai': ['eecvesu@gmail.com', 'eecvarachacanada@gmail.com', 'eecparvatpatiya@gmail.com', 'eeccouns57@gmail.com', 'eeccouns026@gmail.com'], // Same as UAE (alternative name)

  'Ireland': ['eecnikolvisa@gmail.com', 'eeccouns005@gmail.com', 'eecvesu@gmail.com'], // TODO: Add Ireland country admin email(s)

  'France': ['eeccouns48@gmail.com', 'eecvesu@gmail.com'], // TODO: Add France country admin email(s)

  'Malta': ['eecapply14@gmail.com', 'eecghatlodiyavisa@gmail.com'], // TODO: Add Malta country admin email(s)
};

// Helper function to get country admin emails for a given country
const getCountryAdminEmails = (country: string): string[] => {
  if (!country) return [];
  
  // Normalize country name (trim and handle alternative names)
  const normalizedCountry = country.trim();
  
  // Check direct match
  if (COUNTRY_ADMIN_EMAILS[normalizedCountry]) {
    return COUNTRY_ADMIN_EMAILS[normalizedCountry];
  }
  
  // Handle alternative names
  if (normalizedCountry === 'Deutschland' || normalizedCountry === 'Germany') {
    return COUNTRY_ADMIN_EMAILS['Germany'] || COUNTRY_ADMIN_EMAILS['Deutschland'] || [];
  }
  
  if (normalizedCountry === 'Dubai') {
    return COUNTRY_ADMIN_EMAILS['UAE'] || COUNTRY_ADMIN_EMAILS['Dubai'] || [];
  }
...
export const getCounselorEmailsFromBranch = (branchIdentifierOrName: string): string[] => {
  ...
  return branch.counselors
    .map((counselor: any) => counselor.email)
...
// NOTE: Currently unused - kept for potential future use with sendCareerInsightNotificationEmails
/*
...
// Build email body for visa search query
const buildVisaSearchEmailBody = (userData: any, searchParams: { destination: string; originState: string; visaType: string }): string => {
  let body = `New Visa Search Query - User Information\n\n`;
  ...
  // Visa Search Query Details
  body += "\n\nVISA SEARCH QUERY / BOOKING DETAILS\n";
  body += "────────────────────────────────────────\n";
  body += `Booking From: India\n`;
  body += `Booking To: ${searchParams.destination || 'N/A'}\n`;
  body += `Origin State: ${searchParams.originState || 'N/A'}\n`;
  body += `Visa Type: ${searchParams.visaType || 'N/A'}\n`;
  
  // Query timestamp
  const queryDate = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
  body += `Search Date: ${queryDate}\n`;
  ...
  body += "2. Select \"Filter messages like this\"\n";
  body += "3. Click \"Create filter\"\n";
  body += `4. Check "Apply the label" and select/create "Visa Search Queries"\n`;
  body += "5. Click \"Create filter\"\n";
  
  return body;
};

// Send visa search notification emails
// Fetches user data from Firestore and sends to admins and branch counselors (if branch selected)
export const sendVisaSearchNotificationEmails = async (userEmail: string, searchParams: { destination: string; originState: string; visaType: string }): Promise<boolean> => {
  try {
    ...
    // Add country-specific admin emails if target country is provided
    const country = searchParams.destination;
      if (country) {
        const countryAdminEmails = getCountryAdminEmails(country);
        if (countryAdminEmails.length > 0) {
          recipientEmails.push(...countryAdminEmails);
        }
      }
    ...
    const emailBody = buildVisaSearchEmailBody(userData, searchParams);
    
    // Build subject line
    const fromCity = userData.city ? `${userData.city}, ` : '';
    const subject = `Visa Search: ${fromCity}${userData.state || 'India'} → ${searchParams.destination} (${searchParams.visaType}) - ${userData.name || 'User'}`;
    ...
    try {
      ...
      console.error('Error sending visa search emails:', error);
      return false;
    }
  } catch (error) {
    console.error('Error in sendVisaSearchNotificationEmails:', error);
    return false;
  }
};
