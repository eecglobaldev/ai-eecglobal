...
const calculateStreak = (dates: Date[]): number => {
    ...
    if (differenceInDays(new Date(), new Date(uniqueDays[0])) > 1) return 0;
    let streak = 1;
    for (let i = 0; i < uniqueDays.length - 1; i++) {
        if (differenceInDays(new Date(uniqueDays[i]), new Date(uniqueDays[i + 1])) === 1) {
            streak++;
        } else {
            break;
        }
...
const processAnalytics = (
    ...
    prepData.forEach((p) => {
        // Only count prep data that has key talking points (not just metadata)
        if (p.keyTalkingPointsUrl || p.type === 'kpoints' || (!p.type && p.keyTalkingPointsUrl)) {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const prepData: PrepData[] = prepDataSnapshot.docs.map(doc => {
                    ...
                    const mapped: PrepData = {
                        ...data,
                        id: doc.id,
                        createdAt: getSafeTimestampString(data.createdAt || data.timestamp)!,
                        // If old format (has keyTalkingPointsUrl), treat as 'kpoints' type
                        type: data.type || (data.keyTalkingPointsUrl ? 'kpoints' : undefined),
                        url: data.url || data.keyTalkingPointsUrl,
                    };
                    return mapped;
