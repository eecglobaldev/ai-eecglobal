...
import { checkUserExists, getUserByPhone, signInWithHiddenAuth } from "../services/userService";
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.type === "success") {
        ...
        // 3️⃣ Sign in with SERVER-SIDE Firebase Auth (Custom Token)
        ...
        // 4️⃣ Store user credentials in localStorage
        localStorage.setItem('AUgsUserEmail', userEmail);
        if ((user as any).name) {
          localStorage.setItem('AUgsUserName', (user as any).name);
        }
