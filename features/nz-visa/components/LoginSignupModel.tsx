...
import { registerUser, checkEmailExists, checkUserExists, ensureUserSignedIn } from '../services/userService';
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userDataToSave = {
        ...formData,
        targetCountry: formData.targetCountry || 'New Zealand', // Default to USA if not set
      ...
      // Store user details locally for downstream flows
      localStorage.setItem('NZUserEmail', formData.email);
      localStorage.setItem('NZUserName', formData.name);
      ...
      try {
        ...
        setError('Registration successful, but authentication failed. Please try signing in.');
        return;
      }
      ...
      // after filling the academic plan form (email will include both registration + academic plan)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-4 sm:p-5 relative max-h-[96vh] overflow-y-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        <form onSubmit={handleRegister} className="mt-2 space-y-3">
          ...
          {formData.isEECAgent === 'Yes' && (
            <div>
              ...
              >
                ...
                {BRANCHES.map((branch: { identifier: string; name: string }) => (
          ...
          {/* VISA TYPE */}
          <div>
            <label className={labelClass}>Which New Zealand Visa are you planning for?<span className="text-red-500 ml-1">*</span></label>
            <select
              name="visaType"
