...
export interface NzApplicationSaveResult {
  success: boolean;
...
/**
 * Save trimmed NZ application profile (only fields from setup form)
 */
export const saveNzApplicationProfile = async (
  userId: string,
  profile: UserProfile
): Promise<NzApplicationSaveResult> => {
  try {
    const timestamp = Date.now();
    const payload = {
      ...profile,
    ...
    await setDoc(applicationRef, cleanedPayload);

    const blob = new Blob([JSON.stringify(profile, null, 2)], {
      type: "application/json",
    });
    const storagePath = `student_sop/${userId}/application_${timestamp}.json`;
    ...
    console.error("Error saving NZ application profile:", error);
    return {
      success: false,
