...
import { checkUserExists, getUserByPhone, ensureUserSignedIn } from "../services/userService";
    ...
    try {
      ...
      if (result.type === "success" || result.message?.toLowerCase().includes("verified")) {
        ...
        if (user && (user as any).email) {
          ...
          localStorage.setItem('NZUserEmail', userEmail);
          
          // Store user name if available
          if ((user as any).name) {
            localStorage.setItem('NZUserName', (user as any).name);
          }
          ...
          try {
            ...
            console.error('Error signing in after login:', authError);
            setError('OTP verified, but authentication failed. Please try again.');
            setIsLoading(false);
            return;
          }
        ...
        {step === "otp" && (
          <form onSubmit={handleVerifyOtp} className="space-y-6">
            ...
            <div className="text-center space-y-2">
              ...
              <button
                type="button"
                className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors text-sm font-medium"
                onClick={() => {
                  setOtp("");
                  setStep("phone");
                  setCountdown(0);
