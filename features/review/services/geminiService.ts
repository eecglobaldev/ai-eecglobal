...
const getApiKey = () => {
  // Try review-specific key first, then fallback to generic key, then try other tool keys
  const apiKey = 
    process.env.NEXT_PUBLIC_GEMINI_API_KEY_REVIEW || 
    process.env.NEXT_PUBLIC_GEMINI_API_KEY || 
    process.env.NEXT_PUBLIC_GEMINI_API_KEY_CAREER_COUNSELOR ||
    process.env.NEXT_PUBLIC_GEMINI_API_KEY_USA_VISA ||
    process.env.NEXT_PUBLIC_GEMINI_API_KEY_TRAVEL_AGENT ||
    process.env.GEMINI_API_KEY;
  
  if (!apiKey) {
    console.error("Gemini API key not found. Please set NEXT_PUBLIC_GEMINI_API_KEY_REVIEW or NEXT_PUBLIC_GEMINI_API_KEY");
