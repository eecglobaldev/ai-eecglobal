...
export const checkUserExists = async (phone: string) => {
  const usersRef = collection(db, "uk_users");
 ...
 * 2. Server creates Firebase Auth user, users_auth, and uk_users entry
 * 3. Update uk_users with additional fields (state name, branch name, etc.)
 * 
...
export const registerUser = async (userData: any) => {
  try {
    ...
    const result = await globalRegisterUser(email, phone, firstName, lastName, passportNumber, 'uk_users');
    ...
    // 2️⃣ UPDATE uk_users WITH ADDITIONAL FIELDS
    ...
    // Update uk_users with additional fields
    const userDocRef = doc(db, "uk_users", uid);
...
export const incrementPrepPlanCount = async (email: string) => {
  try {
    ...
    const userRef = doc(db, "uk_users", userDoc.id);
 ...
 * Passes 'uk_users' as the collection for migration.
 * 
...
export const signInWithHiddenAuth = async (
  ...
  return await globalSignInWithHiddenAuth(email, 'uk_users');
};
